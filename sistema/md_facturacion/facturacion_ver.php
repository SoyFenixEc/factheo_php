...
if ($factura_id <= 0) {
    header("Location: facturacion_lista.php");
    exit;
}
...
$factura = $stmt_factura->fetch();

if (!$factura) {
    die("Factura no encontrada.");
}

// Generar secuencial con ceros
$secuencial = str_pad($factura['secuencial_factura'], 9, '0', STR_PAD_LEFT);
$numero_factura = $factura['establecimiento'] . '-' . $factura['punto_emision'] . '-' . $secuencial;

// Estado del comprobante
$estado = $factura['estado_xml'] ?? 'GENERADO';
$badge_class = match($estado) {
    'GENERADO' => 'badge-info',
    'FIRMADO' => 'badge-warning',
...
// Obtener productos
$sql_detalle = "
    SELECT df.cantidad, df.precio_unitario, df.subtotal, p.nombre AS producto, p.codigo
    FROM detalle_factura df
    JOIN productos p ON df.producto_id = p.id
    WHERE df.factura_id = ?
";
$stmt_detalle = $pdo->prepare($sql_detalle);
$stmt_detalle->execute([$factura_id]);
...
<head>
    ...
    <title>RIDE - Factura #<?php echo $factura['id']; ?></title>
    ...
    <style>
        ...
        @media print {
            .actions, #wrapper, #content-wrapper, #sidebarToggleTop {
                ...
                <div id="dynamic-content" class="container-fluid">
                    <div class="ride-container">
                        ...
                        <div class="ride-header">
                            <img src="../md_empresa/logos/<?php echo $factura['logo']; ?>" alt="Logo" class="ride-logo">
                        ...
                        <div class="info-grid">
                            ...
                            <div class="info-box">
                                ...
                                <div class="info-row"><strong>Nombre:</strong> <?php echo $factura['cliente_nombre']; ?></div>
                                <div class="info-row"><strong>RUC/CI:</strong> <?php echo $factura['cliente_ruc']; ?></div>
                                <div class="info-row"><strong>Dirección:</strong> <?php echo $factura['cliente_direccion']; ?></div>
                                <div class="info-row"><strong>Teléfono:</strong> <?php echo $factura['cliente_telefono']; ?></div>
                            </div>
                        </div>
                        ...
                        <div class="info-grid">
                            <div class="info-box">
                                ...
                                <div class="info-row"><strong>Tipo:</strong> FACTURA</div>
                                <div class="info-row"><strong>Fecha:</strong> <?php echo date('d/m/Y H:i', strtotime($factura['fecha_emision'])); ?></div>
                                <div class="info-row"><strong>Número:</strong> <?php echo $numero_factura; ?></div>
                        ...
                        <div class="totales-box">
                            ...
                            <div class="totales-row"><strong>Descuento:</strong> <span>$<?php echo number_format($factura['subtotal1'] * ($factura['descuento'] / 100), 2); ?></span></div>
                            <div class="totales-row"><strong>Subtotal sin IVA:</strong> <span>$<?php echo number_format($factura['subtotal2'], 2); ?></span></div>
                        ...
                        <?php if (!empty($factura['comentarios'])): ?>
                            <div style="margin: 20px; padding: 10px; border: 1px dashed #ccc; font-style: italic;">
                                <strong>Comentarios:</strong> <?php echo $factura['comentarios']; ?>
                            </div>
                        <?php endif; ?>

                        <!-- Acciones -->
                        <div class="actions">
                            <a href="facturacion_lista.php" class="btn btn-secondary ml-2">
