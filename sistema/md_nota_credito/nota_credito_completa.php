...
$nc_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
require('../md_config/conexion.php');
$stmt = $pdo->prepare("SELECT f.*, c.razon_social FROM facturas f LEFT JOIN clientes c ON f.cliente_id = c.id WHERE f.id = ? AND f.tipo_comprobante_id = '04'");
$stmt->execute([$nc_id]);
$nc_data = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$nc_data) die("<div class='alert alert-danger'>Nota de Crédito no encontrada.</div>");
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div id="dynamic-content" class="container-fluid">
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">⚡ Autorizar Nota de Crédito</h1>
                        <a href="nota_credito_lista.php" class="btn btn-sm btn-secondary"><i class="fas fa-arrow-left"></i> Volver</a>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="card shadow-sm p-4">
                                <p class="text-center text-muted">Proceso completo: Generar XML → Firmar → Enviar al SRI (Pruebas) → Autorizar</p>
                                <input type="hidden" id="nc_id" value="<?= $nc_id ?>">
                                <div class="alert alert-info">
                                    <strong>ID:</strong> <?= $nc_id ?><br>
                                    <strong>Cliente:</strong> <?= htmlspecialchars($nc_data['razon_social'] ?? '') ?><br>
                                    <strong>Clave de Acceso:</strong> <?= $nc_data['clave_acceso'] ?? 'No generada' ?><br>
                                    <strong>Estado:</strong> <?= $nc_data['estado_xml'] ?? 'PENDIENTE' ?>
                                </div>
    ...
    async function ejecutar() {
        ...
        try {
            ...
            const r1=await fetch('nota_credito_generar_xml_1_1_0.php?id='+id);
            ...
            document.getElementById('resultado').innerHTML+='<div class="alert alert-success mt-3"><strong>Nota de Crédito AUTORIZADA</strong><br>Número de autorización: '+d4[2]+'</div>';
            Swal.fire({title:'Proceso completado',text:'Nota de Crédito autorizada correctamente',icon:'success'});
