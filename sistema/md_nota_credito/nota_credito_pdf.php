...
$stmt = $pdo->prepare("SELECT f.*, e.logo, e.razon_social, e.nombre_comercial, e.ruc, e.direccion, e.contribuyente_especial, e.obligado_contabilidad FROM facturas f JOIN empresa e ON f.empresa_id = e.id WHERE f.id = ? AND f.tipo_comprobante_id = '04' AND f.estado_xml = 'AUTORIZADO'");
$stmt->execute([$id]);
...
$pdf->SetFont('', '', 7);
$pdf->MultiCell(21, 0, (string)($xml->infoNotaCredito->contribuyenteEspecial ?? 'N/A'), 0, '', 0, 0, 57, 93, true);
$pdf->SetFont('', 'B', 7);
$pdf->MultiCell(50, 0, "OBLIGADO CONTABILIDAD: ", 0, '', 0, 0, 10, 97, true);
$pdf->SetFont('', '', 7);
$pdf->MultiCell(21, 0, (string)$xml->infoNotaCredito->obligadoContabilidad, 0, '', 0, 0, 57, 97, true);
...
$pdf->SetFont('', '', 9);
$pdf->MultiCell(70, 10, (string)$xml->infoNotaCredito->razonSocialComprador, 0, '', 0, 0, 62, 103, true);
$pdf->MultiCell(35, 0, (string)$xml->infoNotaCredito->identificacionComprador, 0, '', 0, 0, 163, 103, true);
$pdf->MultiCell(35, 0, (string)$xml->infoNotaCredito->fechaEmision, 0, '', 0, 0, 45, 114, true);
$pdf->SetFont('', 'B', 8);
$pdf->MultiCell(35, 0, "Factura Modificada: ", 0, '', 0, 0, 100, 114, true);
$pdf->SetFont('', '', 8);
$pdf->MultiCell(60, 0, (string)$xml->infoNotaCredito->numDocModificado, 0, '', 0, 0, 150, 114, true);
...
$pdf->Ln(2);
$iva_val = 0;
foreach ($xml->infoNotaCredito->totalConImpuestos->totalImpuesto as $ti) {
...
$pdf->Cell(20, 7, (string)$xml->infoNotaCredito->totalSinImpuestos, 1, 0, 'R');
$pdf->Ln();
$pdf->SetFont('', '', 8);
$pdf->Cell(131, 7, '     ' . (string)$xml->infoNotaCredito->motivo, 'LR', 0, 'L');
...
$pdf->Cell(20, 7, (string)$xml->infoNotaCredito->valorModificacion, 1, 0, 'R');
