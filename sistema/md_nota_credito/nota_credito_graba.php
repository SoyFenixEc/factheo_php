...
try {
    ...
    $clave_acceso = generarClaveAcceso($fecha_emision, '04', $empresa['ruc'], $punto['establecimiento'], $punto['punto_emision'], $nuevo_secuencial);
    ...
    header("Location: nota_credito_lista.php?msg=success&id=$factura_id");
    exit;

} catch (Exception $e) {
    if (isset($pdo) && $pdo->inTransaction()) $pdo->rollback();
    error_log("Error en nota_credito_graba.php: " . $e->getMessage());
    header("Location: nota_credito_lista.php?msg=error&error=" . urlencode($e->getMessage()));
    exit;
}
