...
if (!$nc) die("<div class='alert alert-danger'>Nota de Crédito no encontrada.</div>");
...
<!DOCTYPE html>
<html lang="es">
<head><?php require('../entorno/meta.php'); require('../entorno/title.php'); require('../entorno/link.php'); ?><title>Nota de Crédito #<?= $num ?></title></head>
<body>
    <div id="wrapper">
            ...
            <div id="content">
                ...
                <div id="dynamic-content" class="container-fluid">
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Nota de Crédito #<?= $num ?></h1>
                        <a href="nota_credito_lista.php" class="btn btn-sm btn-secondary"><i class="fas fa-arrow-left"></i> Volver</a>
<a href="nota_credito_pdf.php?id=<?= $id ?>" class="btn btn-sm btn-danger" target="_blank"><i class="fas fa-file-pdf"></i> PDF</a> <a href="nota_credito_pdf.php?id=<?= $id ?>" class="btn btn-sm btn-danger" target="_blank"><i class="fas fa-file-pdf"></i> PDF</a>
                    </div>

                    <?php if ($nc['estado_xml'] == 'AUTORIZADO'): ?>
                        <div class="alert alert-success"><i class="fas fa-check-circle"></i> AUTORIZADA - Nro: <?= $nc["numero_autorizacion"] ?? "Pendiente" ?> <a href="nota_credito_pdf.php?id=<?= $id ?>" class="btn btn-sm btn-danger float-right" target="_blank"><i class="fas fa-file-pdf"></i> Descargar PDF</a></div>
                    <?php endif; ?>
                    ...
                    <div class="text-center mb-4">
                        <a href="nota_credito_completa.php?id=<?= $nc['id'] ?>" class="btn btn-success btn-lg"><i class="fas fa-check"></i> AUTORIZAR</a>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
